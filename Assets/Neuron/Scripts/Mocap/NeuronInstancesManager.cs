...
public class NeuronInstancesManager : MonoBehaviour
{
	...
	public bool Connect( string address, int bvhPort, int calcPort, int commandServerPort, NeuronConnection.SocketType socketType )
	{
		NeuronSource source =  NeuronConnection.Connect( address, bvhPort, calcPort, commandServerPort, socketType );
		if( source != null )
		{
	...
	public void Disconnect( string address, int bvhPort, int calcPort )
	{
		NeuronSource source = null;
		for( int i = 0; i < sources.Count; ++i )
		{
			if( address == sources[i].address && bvhPort == sources[i].bvhPort && calcPort == sources[i].calcPort)
			{
				source = sources[i];
				break;
			}
	...
	public NeuronSource FindSource( string address, int bvhPort, int calcPort )
	{
		for( int i = 0; i < sources.Count; ++i )
		{
			if( sources[i].address == address && sources[i].bvhPort == bvhPort && sources[i].calcPort == calcPort)
			{
				return sources[i];
			}
		}

...
namespace Neuron
{
	public class NeuronSource
	{
		...
		public int											bvhPort { get; private set; }
        public int                                          calcPort { get; private set; }
        public int											commandServerPort{ get; private set; }
		public NeuronConnection.SocketType					socketType { get; private set; }
		public IntPtr										bvhSocketReference { get; private set; }
        public IntPtr                                       calcSocketReference { get; private set; }
		...
		public NeuronSource( string address, int bvhPort, int calcPort, int commandServerPort, NeuronConnection.SocketType socketType, IntPtr bvhSocketReference, IntPtr calcSocketReference, IntPtr commandSocketReference )
		{
			this.address = address;
			this.bvhPort = bvhPort;
            this.calcPort = calcPort;
            this.commandServerPort = commandServerPort;
			this.socketType = socketType;
			this.bvhSocketReference = bvhSocketReference;
            this.calcSocketReference = calcSocketReference;
		...
		public void OnUpdate()
		{
			...
			{
				if( now - iter.Value.bvhtimeStamp > NoDataFrameTimeOut )
				{
        ...
        public virtual void OnCalculationDataReceived(IntPtr DataHeader, IntPtr data)
        {
            CalcDataHeader header = new CalcDataHeader();
            try
            {
                header = (CalcDataHeader)Marshal.PtrToStructure(DataHeader, typeof(CalcDataHeader));
            }
            catch (Exception e)
            {
                Debug.LogException(e);
            ...
            {
                // if actor is active
                actor.OnReceivedCalculationData(header, data);
            }
            else
            {
                // find suspended actor

...
public class DebugMenu : NeuronInstancesManager
{
	...
	public InputField BvhPortField;
    public InputField CalcPortField;
	...
	public void AddConnection()
	{
		...
		int bvhPort = int.Parse (BvhPortField.text);
        int calcPort = int.Parse(CalcPortField.text);

        NeuronSource source = FindSource( address, bvhPort, calcPort );
		if( source != null )
		{
			if ( source.numOfActiveActors == 0 )
			{
				Disconnect( source.address, source.bvhPort, source.calcPort );
			} 
			else 
			{
				Debug.Log( string.Format( "[NeuronDebugViewer] Connection to {0}:{1} already present.", address, bvhPort ) );
                Debug.Log(string.Format("[NeuronDebugViewer] Connection to {0}:{1} already present.", address, calcPort));
                return;
			}
		}
		
		NeuronConnection.SocketType socketType = UDPToggle.isOn ? NeuronConnection.SocketType.UDP : NeuronConnection.SocketType.TCP;
		Connect( address, bvhPort, calcPort, -1, socketType );
	}
	...
	public void RemoveConnection()
	{
		...
		int bvhPort = int.Parse (BvhPortField.text);
        int calcPort = int.Parse(CalcPortField.text);
        Debug.Log( string.Format( "[NeuronDebugViewer] Remove connection {0}:{1}", address, bvhPort ) );
        Debug.Log(string.Format("[NeuronDebugViewer] Remove connection {0}:{1}", address, calcPort));
        Disconnect( address, bvhPort, calcPort);
	}
